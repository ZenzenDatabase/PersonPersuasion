...
# model = TransformerRegression(input_dim, output_dim, num_heads, num_layers, hidden_dim)

# PATH='../DPPR/DPPR_model.pt'

# model = torch.load(PATH)
# latent = model(t_oneturn.to(torch.float32).to("cuda:0")).detach().numpy()

# this_state_action = torch.rand((1, 768))
# pred_this_state_action = model(t_state.to(torch.float32).to(device))
# print(pred_this_state_action.shape)
